...
/*                                       CASE 3 STARTS HERE                                       */
	...
	async fetchAssignedPapers(reviewerId) {
		console.log(`[INFO] Fetching assigned papers for reviewerId: ${reviewerId}`);
		...
		const assignedPapers = papers.filter((paper) =>
    		paper.reviewers && paper.reviewers.some((reviewer) => reviewer.id === reviewerId)
		);


		console.log(`[INFO] Assigned Papers: ${JSON.stringify(assignedPapers)}`);
	
		const assignedPapersContainer = document.querySelector("#assigned-papers");
	...
	async loadReviewForm(paperId, reviewerId) {
		...
		if (review) {
			...
			document.querySelector(`input[name="paper-contribution"][value="${review.contribution}"]`).checked = true;
			...
			document.querySelectorAll('input[name="overall-evaluation"]').forEach((input) => (input.checked = false));
			document.querySelectorAll('input[name="paper-contribution"]').forEach((input) => (input.checked = false));
...
/*                                       EVENT LISTENERS                                       */
	...
	document.addEventListener("DOMContentLoaded", async () => {
		...
		if (submitPaperForm) {
			...
			document.querySelector("#add-author").addEventListener("click", async (e) => {
				...
				if (authorDetails.ok) {
					...
					console.log(`[INFO] Added Authors: ${addedAuthors}`);
		...
		reviewForm.addEventListener("submit", async (e) => {
			...
			const overallEvaluationInput = document.querySelector('input[name="overall-evaluation"]:checked');
			const overallEvaluation = overallEvaluationInput ? parseInt(overallEvaluationInput.value) : null;
			const paperContribution = parseInt(document.querySelector('input[name="paper-contribution"]:checked').value);
